...
def wr_inputs(wr_one,wr_two,position):
    ...
    pf.add_position(wr_one,wr_one_pos)
    pf.add_position(wr_two,wr_two_pos)
...
def wr_route_gen(wr_one,wr_two,time):
    ...
    if wr_one['status'][-1] == 'Vert':
        wr_one_coord = [wr_one['position'][-1][0],102]
        wr_one_x,wr_one_y,wr_one_z = pf.p_to_p_in_t(time,wr_one,wr_one_coord)
        
    if wr_two['status'][-1] == 'Vert':
        wr_two_coord = [wr_two['position'][-1][0],102]
        wr_two_x,wr_two_y,wr_two_z = pf.p_to_p_in_t(time,wr_two,wr_two_coord)
